...
class AdminController < ApplicationController
  ...
  # POST /admin/backfill_structured_transcripts
  # Backfill structured_transcript for videos that have transcript but no structured data
  def backfill_structured_transcripts
    limit = [params[:limit]&.to_i || 100, 500].min
    ...
    if pending == 0
      return render json: {
        success: true,
        message: "All videos already have structured transcripts",
        pending: 0
      }
    end

    # Process inline (not Sidekiq) for small batches, enqueue for large
    if limit <= 5
      results = backfill_structured_inline(scope.limit(limit).order(:id))
      render json: { success: true, **results, pending: pending }
    else
      BackfillStructuredTranscriptsJob.perform_async(limit, language)
      render json: {
        success: true,
  ...
  # POST /admin/extract_clips
  # Extract exercise clips from videos with structured_transcript
  def extract_clips
    limit = [params[:limit]&.to_i || 10, 200].min

    videos_scope = YoutubeVideo
      .where.not(structured_transcript: nil)
      .left_joins(:exercise_video_clips)
      .where(exercise_video_clips: { id: nil })
    pending = videos_scope.count

    if pending == 0
      return render json: {
        success: true,
        message: "All videos with structured transcripts already have clips extracted",
        pending: 0
      }
    end

    if limit <= 5
      results = extract_clips_inline(videos_scope.limit(limit).order(:id))
      render json: { success: true, **results, pending: pending }
    else
      ExtractClipsJob.perform_async(limit)
      render json: {
        success: true,
        message: "Clip extraction started in background",
        pending: pending,
        processing_limit: limit
      }
    end
  ...
  # GET /admin/clip_stats
  # Show exercise video clip statistics
  def clip_stats
    total_clips = ExerciseVideoClip.count
    total_videos_with_clips = ExerciseVideoClip.distinct.count(:youtube_video_id)
    total_videos_with_structured = YoutubeVideo.where.not(structured_transcript: nil).count
    total_videos = YoutubeVideo.count

    by_type = ExerciseVideoClip.group(:clip_type).count
    by_muscle = ExerciseVideoClip.group(:muscle_group).count
    by_language = ExerciseVideoClip.group(:source_language).count
    top_exercises = ExerciseVideoClip.group(:exercise_name).count.sort_by { |_, v| -v }.first(20)

    render json: {
      total_clips: total_clips,
      total_videos_with_clips: total_videos_with_clips,
      total_videos_with_structured: total_videos_with_structured,
      total_videos: total_videos,
      by_clip_type: by_type,
      by_muscle_group: by_muscle,
      by_language: by_language,
      top_exercises: top_exercises.to_h
    }
  end
  ...
  def backfill_structured_inline(videos)
    success = 0
    failed = 0
    results = []
    ...
    videos.find_each do |video|
      ...
      if structured.present?
        ...
        success += 1
        results << { video_id: video.video_id, title: video.title.truncate(40), captions: structured.length }
      else
        failed += 1
        results << { video_id: video.video_id, title: video.title.truncate(40), error: "no subtitles" }
      end
    rescue StandardError => e
      failed += 1
      results << { video_id: video.video_id, error: e.message.truncate(80) }
    end

    { success: success, failed: failed, results: results }
  end

  def extract_clips_inline(videos)
    success = 0
    failed = 0
    results = []
    ...
    videos.find_each do |video|
      ...
      results << {
        video_id: video.video_id,
        title: video.title.truncate(40),
        clips_count: clips.length,
        clips: clips.map { |c| { exercise: c.exercise_name, type: c.clip_type, title: c.title } }
      }
    rescue StandardError => e
      failed += 1
      results << { video_id: video.video_id, error: e.message.truncate(80) }
