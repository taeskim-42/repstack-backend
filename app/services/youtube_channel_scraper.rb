...
class YoutubeChannelScraper
  ...
  class << self
    ...
    # Returns transcript text with timestamps or nil if not available
    ...
    # Format transcript snippets to timestamped text
    def format_transcript(transcript)
      lines = transcript.map do |snippet|
        start_seconds = snippet["start"].to_i
        text = snippet["text"].to_s.gsub(/\s+/, " ").strip
        next if text.empty?

        "[#{format_seconds(start_seconds)}] #{text}"
      end.compact
